...
function FirstCreedItem() {
  ...
  return (
    ...
    >
      ...
      <div ref={textRef} className="flex-1 w-full max-w-lg">
        <blockquote className="text-white text-base md:text-lg leading-loose mb-6 whitespace-pre-line">
          「すべての行動には結果が伴う。<br />　それが己が意図しなかった結果だとしても。」
        </blockquote>
  ...
  return (
    ...
    >
      ...
      <div ref={textRef} className="flex-1 w-full max-w-lg">
        ...
        <p className="text-white/55 text-sm leading-[2] whitespace-pre-line">
          謙虚でいることは大切にしていますが、謙虚を意識しすぎると「自分にできるだろうか」と迷い続けた結果、肝心な場面で退いてしまう。そんな経験を何度かしてからこの言葉に出会いました。<br /><br />やると決めたことに対しては「全部うまくいく」と自分に言い聞かせるようにしています。根拠がなくてもいい。覚悟を決めた瞬間から動き方が変わる。桓騎のあの笑顔には、そういう強さが宿っている気がします。この言葉は今もお守りになっています。<br /><br />「キングダム」
        </p>
      </div>
    </div>
  )
...
function ThirdCreedItem() {
  ...
  return (
    ...
    >
      ...
      <div ref={textRef} className="flex-1 w-full max-w-lg">
        <blockquote className="text-white text-base md:text-lg leading-loose mb-6 whitespace-pre-line">
          「4年生で私より絵がウマい奴がいるなんて<br />　絶っっ対に許せない」
        </blockquote>
        <p className="text-white/55 text-sm leading-[2] whitespace-pre-line">
